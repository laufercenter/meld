...
class TestGetAA1(unittest.TestCase):
    def test_can_parse_simple_sequence(self):
        ...
        result = parse.get_sequence_from_AA1(contents=content)
    ...
    def test_can_handle_all_residues(self):
        ...
        self.assertEqual(result, 'NALA CYS ASP GLU PHE GLY HIE ILE LYS LEU MET ASN PRO GLN ARG SER THR VAL TRP CTYR')

    def test_should_raise_with_bad_residue(self):
        content = 'AX'

        with self.assertRaises(RuntimeError):
            parse.get_sequence_from_AA1(contents=content)
